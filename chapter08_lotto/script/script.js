...
// // 배열에 값 대입 방식 ->  해당 부분은 java와 좀 차이가 있기 때문에 유심히 봐주세요.
...
// console.log(arr02);   // 수박 제외하고 동일한 배열 출력

// // 특정 index에 element 넣기
// arr02[1] = '딸기';
// console.log(arr02); // ['사과', '딸기', '바나나', '키위']
// // 이상의 코드에서 확인해봤을때 '오렌지'가 '딸기'로 대체되었음을 확인 가능. 즉 index를 한 칸씩 민다거나 하는건 아닙니다.


// 이제 배열을 사용해서 실질적으로 로또 번호 생성기를 구현할 예정

// 1. lottoArray 배열을 선언하고
// 2. 해당 배열 내에 1~45까지의 number 값을 넣을겂니다.
// 3. 그리고 .pop()해서 하나씩 뽑아서 출력하면 어느정도 구현되지 않을까요?
...
// 이상에서의 코드가 java와의 약간 차이점은 있지만 이전에 웹1에서 구현했던 로또 번호 생성기와 별 차이가 없습니다. -> 구현 방식은 좀 다르지만
